...
describe('Mocking delivery options request', () => {
  ...
  ${saturday} | ${monday}   | ${0}         | ${'14:30'}  | ${7}                | ${'1;2;3;4;5;6'} | ${1}
  ...
  `('MyParcel: the next delivery date for $day is $result', ({ day, result, dropOffDelay, ...args }) => {
    ...
    const deliveryOption = getNextDeliveryOption({ ...args, platform: MYPARCEL }, dropOffDelay + 1);
  ...
  ${saturday} | ${monday}   | ${0}         | ${'14:30'}  | ${7}                | ${'0;1;2;3'}     | ${0}
  ${tuesday}  | ${thursday} | ${1}         | ${'09:30'}  | ${7}                | ${'1;2;3;4;5;6'} | ${0}
  `('SendMyParcel: the next delivery date for $day is $result', ({ day, result, dropOffDelay, ...args }) => {
    MockDate.set(`${day}T10:00:00`);
    const deliveryOption = getNextDeliveryOption({ ...args, platform: SENDMYPARCEL }, dropOffDelay + 1);
    const date = deliveryOption.data.date.date.replace(' 00:00:00.000000', '');
    expect(date).toMatch(result);
  });
});
