...
import {RENDER_DELIVERY_OPTIONS, UPDATE_DELIVERY_OPTIONS} from '@myparcel/delivery-options-shared';
import Vue, {ComponentPublicInstance, createApp} from 'vue';
 ...
 */
const loadApp = (event, selector = baseSelector) => {
...
function renderApp(event, selector = baseSelector) {
  ...
  const app = createApp(DeliveryOptions);
