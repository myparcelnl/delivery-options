...
import {BPOST, CONFIG, DHL_FOR_YOU, KEY_CONFIG, PLATFORMS, POSTNL, UPDATED_DELIVERY_OPTIONS} from '@myparcel-do/shared';
...
const CUSTOM_CARRIER_IDENTIFIER = `${DHL_FOR_YOU}:12345`;
...
describe('carriers', () => {
  it('shows one carrier', async () => {
    ...
    expect(wrapper.findChoice('carrier', DHL_FOR_YOU).exists()).toBe(false);
    expect(postnl.exists()).toBe(true);
    expect(wrapper.findChoice('carrier', DHL).exists()).toBe(false);
    expect(event.detail.carrier).toEqual(POSTNL);
  });
  ...
  it('can show multiple carriers', async () => {
    ...
    const postNl = wrapper.findChoice('carrier', POSTNL);
    const dhlForYou = wrapper.findChoice('carrier', DHL_FOR_YOU);
  ...
  it('shows a carrier logo when there is a single carrier', async () => {
    ...
    carrierHasImage(wrapper, POSTNL);
  });
  ...
  it('shows carrier logos when there are multiple carriers', async () => {
    ...
    carrierHasImage(wrapper, POSTNL);
    carrierHasImage(wrapper, DHL_FOR_YOU);
  });
